...
LLAMA_BENCH_RELEASE_EXE = Path(
    "BONUS-llama-cpp-optimization/llama.cpp/build/bin/Release/llama-bench.exe"
)

# llama-bench prints a markdown-ish table; decode row is usually tg64/tg128.
TG_RE = re.compile(r"\|\s*tg\d+\s*\|\s*([0-9]+(?:\.[0-9]+)?)")


def find_bench() -> Path:
    for p in (LLAMA_BENCH, LLAMA_BENCH_EXE, LLAMA_BENCH_RELEASE_EXE):
        if p.exists():
            return p
